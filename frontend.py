...
def landing_page():
    empty_data()
    global login_f,home_i1, logo_i1, home_i2, home_f, home_c, home_b1, home_b2, home_b3, home_b4, interview_f,scrollable,scrollable1
    scrollable.destroy()
    interview_f.destroy()
    scrollable1.destroy()
    login_f.destroy()
...
def login():
    global login_f,logo_i1
    logo_i1=PhotoImage(file = "logo.png")
    login_f = Frame(root, bg="#FAFAFA")
    login_f.pack()
    login_c=Canvas(login_f,height=1080,width=1920,bg='#FAFAFA')
    login_c.pack(fill = "both", expand = True)
    login_c.create_rectangle(0, 0, 1920, 100, fill="#4EC261", outline="#4EC261")
    # login_c.create_rectangle(0, 100, 300, 1080, fill="#1F7D2E", outline="#1F7D2E")
    login_v1=tkinter.StringVar()
    login_v2=tkinter.StringVar() 
    def login_check():
        c.execute("select * from users where username='"+login_e1.get()+"' and password='"+login_e2.get()+"'")
        val=c.fetchall()
        if(len(val)==0):
            tkinter.messagebox.showinfo("Error","Invalid Username")
        else:
            landing_page()
    login_c.create_image(100,15,image=logo_i1,anchor="nw")
    login_rect=CTkFrame(login_c,fg_color="#FAFAFA",height=510,width=800,border_width=2,corner_radius=100)
    login_rect.place(x=400,y=190)
    login_e1=CTkEntry(login_c,height=50,width=300,fg_color="#FAFAFA",border_color="#1F7D2E",text_color="#4EC261",corner_radius=20,placeholder_text="Username",placeholder_text_color="#4EC261",font=("Arial", 20),justify="center")
    login_e1.place(x=650,y=300)
    login_e2=CTkEntry(login_c,height=50,width=300,fg_color="#FAFAFA",border_color="#1F7D2E",text_color="#4EC261",corner_radius=20,placeholder_text_color="#4EC261",placeholder_text="Password",font=("Arial", 20),show="*",justify="center")
    login_e2.place(x=650,y=400)
    login_c.create_text(750, 150, text="LOGIN", font=("Arial", 60))
    login_b1=CTkButton(login_c,text="Login",font=("Arial", 30),width=280,height=50,fg_color="#4EC261",border_color="#4EC261",text_color="white",cursor="hand2",hover_color='#1F7D2E',command=login_check)
    login_b1.place(x=670,y=500)
    # login_l1=CTkLabel(login_c,text="signup",text_color="gray",font=("Arial", 20),fg_color="#FAFAFA")
    # login_l1.place(x=500,y=300)
def past_scores():
